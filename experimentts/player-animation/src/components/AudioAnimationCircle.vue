  ...
  mounted() {
    ...
    var Controls = {
      ...
      getQuadrant() {
        ...
        if (
          (Math.PI * 3) / 2 <= Tracker.angle &&
          Tracker.angle <= Math.PI * 2
        ) {
          return 4
        }
      },
    ...
    var Player = {
      ...
      tracks: [
        {
          ...
          url: 'https://file-ovlcecvpit.now.sh/', // url: 'http://localhost:8081/colors.mp3',
        },
      ],
      ...
      async loadTrack(index) {
        ...
        const arrayBuffer = await fetch(track.url, {
          mode: 'cors',
        }).then(res => res.arrayBuffer())
        console.log(arrayBuffer)
        this.source.buffer = await that.context.decodeAudioData(arrayBuffer)
